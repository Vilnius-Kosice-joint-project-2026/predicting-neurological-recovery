...
MODEL_OUTPUT_ROOT = HPC_ROOT / 'artifacts' / 'model_outputs_607'
...
for fold, (train_idx, test_idx) in enumerate(skf.split(patient_meta, patient_meta['Outcome'])):
    ...
    checkpoint_path = MODEL_OUTPUT_ROOT / f'best_model_B0_{fold_suffix}.keras'
    history_path = MODEL_OUTPUT_ROOT / f'history_B0_{fold_suffix}.csv'
    oof_save_path = MODEL_OUTPUT_ROOT / f'oof_predictions_cnn2_grid.csv'
    ...
    patient_res = (
        res_df.groupby(['Patient', 'label'])['prob_poor']
        .agg(
            prob_poor_mean='mean',
            prob_poor_std='std',
            prob_poor_min='min',
            prob_poor_max='max',
            prob_poor_median='median',
            prob_poor_q25=lambda s: s.quantile(0.25),
            prob_poor_q75=lambda s: s.quantile(0.75),
            prob_poor_count='count',
            prob_poor_frac_gt_05=lambda s: (s > 0.5).mean(),
        )
        .reset_index()
    )
    patient_res['prob_poor_std'] = patient_res['prob_poor_std'].fillna(0.0)
    patient_res['prob_poor_iqr'] = patient_res['prob_poor_q75'] - patient_res['prob_poor_q25']
    patient_res['prob_poor_confidence'] = (patient_res['prob_poor_mean'] - 0.5).abs()
    patient_res['fold'] = fold + 1

    oof_predictions_list.append(patient_res)

    auc = roc_auc_score(patient_res['label'], patient_res['prob_poor_mean'])
